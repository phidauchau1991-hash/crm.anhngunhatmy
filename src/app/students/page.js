...
export default function StudentsPage() {
  ...
  // Submit chỉnh sửa học viên (Phân loại theo Tab)
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    setSubmitting(true);
    setMessage({ type: '', text: '' });

    let payload = { action: editTab };
            ...
            {loadingEditData ? (
              ...
              <>
                ...
                <div className="modal-tabs">
                  <button className={`tab-btn ${editTab === 'basic' ? 'active' : ''}`} onClick={() => setEditTab('basic')}>1. Chỉnh sửa thông tin</button>
                  <button className={`tab-btn ${editTab === 'payment' ? 'active' : ''}`} onClick={() => setEditTab('payment')}>2. Thanh toán / Thu tiền</button>
                  <button className={`tab-btn ${editTab === 'statusChange' ? 'active' : ''}`} onClick={() => setEditTab('statusChange')}>3. Đang học / Tạm nghỉ / Nghỉ luôn</button>
                  <button className={`tab-btn ${editTab === 'transfer' ? 'active' : ''}`} onClick={() => setEditTab('transfer')}>4. Xếp lớp / Chuyển lớp</button>
                  <button className={`tab-btn ${editTab === 'reserve' ? 'active' : ''}`} onClick={() => setEditTab('reserve')}>5. Bảo lưu</button>
                  <button className={`tab-btn ${editTab === 'dropout' ? 'active' : ''}`} onClick={() => setEditTab('dropout')}>6. Nghỉ luôn</button>
                  <button className={`tab-btn ${editTab === 'gift' ? 'active' : ''}`} onClick={() => setEditTab('gift')}>7. Cấp phát vật tư</button>
                </div>
                ...
                {editTab === 'payment' ? (
                  ...
                  <form onSubmit={handleEditSubmit} className="modal-form">
                    ...
                    {editTab !== 'gift' && (
                      <div className="modal-actions">
                        <button type="button" className="btn btn-secondary" onClick={() => setIsEditModalOpen(false)}>Đóng</button>
                        <button type="submit" className="btn btn-primary" disabled={submitting}>Xác nhận Nghiệp vụ</button>
                      </div>
                    )}
                  </form>
                )}
