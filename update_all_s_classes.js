...
const dobs = {
  ...
  'TRẦN NGUYÊN ANH KHÔI': '2018-01-04', // fixed 2028 to 2018
...
const updateDob = db.prepare(`UPDATE Student SET dob = ?, updatedAt = ? WHERE name = ?`);
const nowStr = new Date().toISOString();

let updated = 0;
for (const [name, dateStr] of Object.entries(dobs)) {
  const isoStr = new Date(`${dateStr}T00:00:00.000Z`).toISOString();
  const res = updateDob.run(isoStr, nowStr, name);
  if (res.changes > 0) {
    updated += res.changes;
  }
}
