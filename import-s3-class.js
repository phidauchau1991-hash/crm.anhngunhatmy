...
async function main() {
  console.log('=== EXECUTE IMPORT FOR S3 (LỊCH 35) & 12 HỌC VIÊN ===\n');
  ...
  // 1. Upsert Class
  const classCode = 'CN1_S3_MsMy_35_Ca2';
  const s3ClassRow = classRows.find(r => (r['Mã lớp'] || '').toString().trim() === classCode);
  if (!s3ClassRow) {
    throw new Error('Không tìm thấy mã lớp CN1_S3_MsMy_35_Ca2 trong sheet Lớp Học');
  }

  const level = s3ClassRow['Khóa học']?.toString().trim();
  const teacherName = s3ClassRow['Giáo viên']?.toString().trim();
  const startDate = parseExcelDate(s3ClassRow['Ngày khai giảng']);
  const schedule = s3ClassRow['Lịch học tuần']?.toString().trim();
  const sessionsTaughtInClass = parseInt(s3ClassRow['Số buổi đã học thực tế']) || 0;

  const courseConfig = await prisma.courseConfig.findUnique({ where: { level } });
  const totalSessions = courseConfig ? courseConfig.totalSessions : 36;
  ...
  // 2. Import Students
  const s3Students = studentRows.filter(r => (r['Mã lớp xếp vào'] || '').toString().trim() === classCode);
  console.log(`--- TIẾN HÀNH THÊM 100% ACCURATE ${s3Students.length} HỌC VIÊN ---`);
  ...
  for (let i = 0; i < s3Students.length; i++) {
    const row = s3Students[i];
    ...
    const customFee = parseFloat(row['Học phí thỏa thuận']) || 3450000;
    ...
    const paymentStatus = amountPaid >= feeToPay ? 'Đã đóng' : 'Chưa đóng';
  ...
  console.log('\n=== IMPORT THÀNH CÔNG 100% CẢ 12 HỌC VIÊN VÀO LỚP S3 (35) ===');
}
